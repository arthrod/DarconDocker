...
def environment_tab():    
    ...
    llm_default_urls = {
        ...
        "Ollama": "http://10.147.20.20:11434/v1"
    }
    ...
    with st.form("env_vars_form"):
        ...
        # SUPABASE_API_KEY
        SUPABASE_API_KEY_help = "Get your SUPABASE_API_KEY from the API section of your Supabase project settings -\nhttps://supabase.com/dashboard/project/<your project ID>/settings/api\nOn this page it is called the service_role secret."
        
        # If there's already a value, show asterisks in the placeholder
        placeholder = "Set but hidden" if profile_env_vars.get("SUPABASE_API_KEY", "") else ""
        SUPABASE_API_KEY = st.text_input(
            "SUPABASE_API_KEY:",
            type="password",
            help=SUPABASE_API_KEY_help,
            key="input_SUPABASE_API_KEY",
            placeholder=placeholder
        )
        # Only update if user entered something (to avoid overwriting with empty string)
        if SUPABASE_API_KEY:
            updated_values["SUPABASE_API_KEY"] = SUPABASE_API_KEY
