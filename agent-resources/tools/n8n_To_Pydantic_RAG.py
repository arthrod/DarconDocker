...
When performing a conversion:
  ...
  - Consult both the n8nxpyai documentation and the n8npyaigit table for repository information stored in Supabase.
...
async def retrieve_relevant_documentation(
    ...
    Args:
        ...
        source: Which table to query ("n8npyaigit" or "github").
        
    Returns:
        A formatted string containing the top 5 most relevant chunks.
    """
    logging.debug(f"Retrieving documentation for query: {user_query} from source: {source}")
    try:
        query_embedding = await get_embedding(user_query, ctx.deps.openai_client)
        logging.debug(f"Query embedding: {query_embedding}")
        formatted_chunks = []

        if source == "n8npyaigit":
            # Query the n8npyaigit table using the match_n8npyaigit RPC.
            ...
            logging.debug(f"n8npyaigit result: {result.data}")

            if not result.data:
                return "No relevant n8npyaigit documentation found."
...
"""
        ...
        elif source == "n8npyaigit":
            # Query the n8npyaigit table using the match_n8npyaigit RPC.
            result = ctx.deps.supabase.rpc(
                'match_n8npyaigit_files',
                {
                    'query_embedding': query_embedding,
                    'match_count': 5
                }
            ...
            if not result.data:
                return "No relevant n8npyaigit files found."

            for doc in result.data:
                # Format the n8npyaigit file info.
                chunk_text = f"""
...
"""
            ...
            return f"Unknown source '{source}'. Please specify 'n8npyaigit' or 'github'."
...
async def list_documentation_pages(
    ctx: RunContext[PydanticAIDeps],
    source: str = "n8npyaigit"  # Accepts "n8npyaigit" or "github"
) -> List[str]:
    """
    ...
    try:
        if source == "n8npyaigit":
            # For n8npyaigit, assume the table has a 'url' column.
            result = ctx.deps.supabase.from_('n8npyaigit') \
                .select('url') \
                .execute()
            if not result.data:
                return []
            urls = sorted(set(doc['url'] for doc in result.data))
            logging.debug(f"n8npyaigit URLs: {urls}")
            return urls

        elif source == "n8npyaigit_files":
            # For n8npyaigit_files, build a composite identifier from repo_owner, repo_name, branch, and file_path.
            result = ctx.deps.supabase.from_('n8npyaigit_files') \
            ...
            logging.debug(f"n8npyaigit_files URLs: {urls}")
            return urls
...
async def get_page_content(
    ctx: RunContext[PydanticAIDeps],
    identifier: str,
    source: str = "n8npyaigit"  # Accepts "n8npyaigit" or "n8npyaigit_files"
) -> str:
    """
    ...
    Args:
        ctx: The context including the Supabase client.
        identifier: The URL (for n8npyaigit) or composite file identifier (for n8npyaigit_files).
        source: Which table to query ("n8npyaigit" or "n8npyaigit_files").
        
    Returns:
        A string with the complete page or file content.
    """
    logging.debug(f"Retrieving page content for identifier: {identifier} from source: {source}")
    try:
        if source == "n8npyaigit":
            # For n8npyaigit, retrieve all chunks for a given URL.
            result = ctx.deps.supabase.from_('n8npyaigit') \
            ...
            for chunk in result.data:
                formatted_content.append(f"**Summary:** {chunk['summary']}\n\n{chunk['content']}")
            logging.debug(f"n8npyaigit content: {formatted_content}")
            return "\n\n".join(formatted_content)

        elif source == "n8npyaigit_files":
            # For n8npyaigit_files, parse the composite identifier: "owner/repo/branch/file_path".
            try:
                repo_owner, repo_name, branch, file_path = identifier.split("/", 3)
            except ValueError:
                return "Invalid n8npyaigit_files identifier format. Expected 'owner/repo/branch/file_path'."
...
{doc['file_content']}
"""
            logging.debug(f"n8npyaigit content: {content}")
            return content
...
async def convert_n8n_to_pydantic_ai(
    ...
    if "pydantic" in workflow_text.lower() and "pydantic-ai" not in workflow_text.lower():
    ...
    workflow_outline = f"Outlined functionality of the n8n workflow:\n{workflow_text[:500]}...\n"
    
    # Step 2: Retrieve context from both documentation sources.
    docs_n8n = await retrieve_relevant_documentation(ctx, workflow_text, source="n8npyaigit")
    docs_github = await retrieve_relevant_documentation(ctx, workflow_text, source="n8npyaigit_files")
    ...
    return (
        f"{workflow_outline}\n"
        f"--- Documentation from n8npyaigit ---\n{docs_n8n}\n"
        f"--- Documentation from n8npyaigit_files ---\n{docs_n8npyaigit}\n"
